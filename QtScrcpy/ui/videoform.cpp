...
void VideoForm::contextMenuEvent(QContextMenuEvent *event)
{
    auto device = qsc::IDeviceManage::getInstance().getDevice(m_serial);
    if (!device) {
        return;
    ...
    if (selectedAction == downloadAction) {
        // 弹出对话框让用户输入设备文件路径
        bool ok;
        QString devicePath = QInputDialog::getText(this, 
                                                   tr("Download File"), 
                                                   tr("Enter device file path:"),
                                                   QLineEdit::Normal,
                                                   "/sdcard/Download/",
                                                   &ok);
        if (ok && !devicePath.isEmpty()) {
            // 选择本地保存路径
            ...
            if (!localPath.isEmpty()) {
                device->pullFileRequest(devicePath, localPath);
            }
        }
    }
}
