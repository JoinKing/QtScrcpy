...
public:
    ...
    QStringList listDeviceFiles(const QString &serial, const QString &path);
    QString getStdOut();
    QString getErrorOut();
...
signals:
    ...
    void transferProgress(bool isDownload, int progress);
    void transferLog(const QString &text);

private:
    void initSignals();
    void parseTransferProgress(const QString &text);
...
private:
    ...
    TransferDirection m_transferDirection = TransferDirection::None;
