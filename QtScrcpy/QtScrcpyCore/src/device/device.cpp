...
void Device::initSignals()
{
    ...
    if (m_fileHandler) {
        connect(m_fileHandler, &FileHandler::fileHandlerResult, this, [this](FileHandler::FILE_HANDLER_RESULT processResult, bool isApk, bool isPull) {
            QString tipsType = "";
            if (isApk) {
                tipsType = "install apk";
            } else if (isPull) {
                tipsType = "file download";
            } else {
                tipsType = "file upload";
            }
            QString tips;
...
void Device::pullFileRequest(const QString &devicePath, const QString &localPath)
{
    if (!m_fileHandler) {
        return;
    }
    m_fileHandler->onPullFileRequest(getSerial(), devicePath, localPath);

    for (const auto& item : m_deviceObservers) {
        item->pullFileRequest(devicePath, localPath);
    }
}
